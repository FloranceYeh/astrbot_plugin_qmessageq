...
`<表情>` 可以是表情名（如 `微笑`）、英文别名（如 `smile`）、数字表情 ID（如 `14`）、`#` 加数字（如 `#14`）、数字范围 `a-b`（如 `/face 66-70` 一次回应 66~70 号表情），或一个 Emoji。**打 Emoji 时回应的是字面 Emoji 本身**（如 `/face 💢` 回应一个 `💢`，而非被映射成「发怒」QQ 表情）；打名字/数字时回应对应的 QQ 系统表情。
...
打 Emoji 时（如 `/hface 💢`）会**原样作为文字发出**，不做任何映射；想发任意纯文本用 `/hface text <内容>`。
