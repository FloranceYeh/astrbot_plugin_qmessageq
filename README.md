...
- **回应表情（`/face`）**：引用一条消息后用表情回应（NapCat 的 `set_msg_emoji_like`），支持 `a-b` 范围一次回应多个表情。
...
引用（回复）一条消息后执行 `/parse`，bot 会列出：

- 消息 ID、发送者、时间、文本内容。
- 消息内容里的**表情 ID**（`表情ID: ...`）与**图片 summary**（`图片summary: ...`），后者可用来查看 `/himg` 藏进图片的文案。
- 该消息收到的**表情回应**（`emoji_id`、type、数量）。
- 若消息是**合并转发**，进一步用 `get_forward_msg` 展开每条节点的发送者与内容（最多 30 条）。
