...
- **转发消息伪装（`/fake`）**：发送合并转发消息，伪造节点的昵称与 QQ 号，还可用 `头像=` / `ava=` 自定义节点头像（填 QQ 号自动用其头像）；支持 `||` 分隔多条消息伪造一段群聊记录；消息附带图片时会伪装成第一个人发的图。
...
自定义头像：在节点文本末尾追加 `头像=<图片URL|QQ号>`（别名 `ava=`），如：

```
/fake 张三 10001 你好 头像=https://example.com/avatar.png
/fake 张三 10001 你好 ava=10001
```
填图片 URL 直接用该图作头像；填 QQ 号则自动解析为其头像。值会写入节点的 `avatar` 字段（NapCat / LLOneBot 支持渲染），并从显示文本中移除。文本中请勿以 `头像=` / `ava=` 结尾。
