...
- **发送表情（`/hface`）**：把内置表情（含默认面板不显示的隐藏表情）作为消息发送，支持 `a-b` 范围、名称/ID 及 `/hface list` 查看隐藏表情。
...
范围回应时按 `face_interval`（默认 1 秒）逐个添加，bot 会先发一条「预计耗时」提示，结束后若有失败的表情会汇总成一条失败列表（不会每个失败都单独发一条）。
...
`<名称|ID>` 支持名字 / 英文别名 / 数字 ID / `#ID` / 范围 `a-b`。`/hface` 把内置表情作为普通消息发出，主要用于发送默认表情面板不显示的**隐藏表情**，如 `/hface 骰子`、`/hface 358`；`/hface list` 把全部 110 个隐藏表情以**合并转发消息**的形式发出来（每个节点是一个表情及其名称，点开即可看到实际长相），完整映射见插件目录的 `FACES.md`。
